/** NovelEngine - Ren'Py-inspired VN engine. Pure JS, <20KB. */
(function(W){
'use strict';
...
gpu:function(c,w,h,t){c.fillStyle='#0d0808';c.fillRect(0,0,w,h);for(var i=0;i<6;i++){var px=w*.2+S(t*.5+i)*w*.3,py=h*.3+Co(t*.3+i*.7)*h*.2,g=c.createRadialGradient(px,py,0,px,py,80);g.addColorStop(0,'rgba(228,119,255,.06)');g.addColorStop(1,'rgba(228,119,255,0)');c.fillStyle=g;c.fillRect(0,0,w,h)}c.strokeStyle='rgba(228,119,255,.04)';c.lineWidth=.5;for(var x=0;x<w;x+=20){c.beginPath();c.moveTo(x,0);c.lineTo(x,h);c.stroke()}},
...
dawn:function(c,w,h,t){var hu=270+S(t*.1)*15,g=c.createLinearGradient(0,0,0,h);g.addColorStop(0,'hsl('+hu+',30%,8%)');g.addColorStop(.5,'hsl('+(hu-20)+',25%,7%)');g.addColorStop(1,'hsl('+(hu-40)+',20%,5%)');c.fillStyle=g;c.fillRect(0,0,w,h);stars(c,w,h,t)}
};
var FX={
...
P._dom=function(){
 ...
 T.$sp=mk('img','width:48px;height:48px;border:2px solid rgba(255,255,255,.1);object-fit:cover;float:left;margin-right:12px;display:none;border-radius:0;image-rendering:pixelated',T.$dl);
 T.$sn=mk('div',MF+'font-size:.8rem;font-weight:600;margin-bottom:8px;letter-spacing:1px',T.$dl);
 T.$tx=mk('div',MF+'font-size:.95rem;line-height:1.8;color:#c8c8d0;min-height:3.6em',T.$dl);
...
P._ev=function(){var T=this;T.$dl.addEventListener('click',function(){T._adv()});D.addEventListener('keydown',function(e){if(!T.s.on||T.$mn.style.display==='flex')return;if(e.key===' '||e.key==='Enter'){e.preventDefault();if(!T.$cl.children.length)T._adv()}})};
P._mc=function(id){this.$mn.style.display='none';if(id==='resume')return;if(id[0]==='s'&&id[1]==='a')this._ss(+id[4]);else if(id[0]==='l')this._ls(+id[4])};
...
P._bg=function(k){this.s.bg=k;var T=this;if(T._af)cancelAnimationFrame(T._af);T._at=performance.now();var fn=T.bgs[k]||BG.black;
 ...
 var t=(performance.now()-T._at)/1e3,w=cv.width,h=cv.height;fn(cx,w,h,t);if(T.fx.crt)FX.crt(cx,w,h);if(T.fx.vig)FX.vignette(cx,w,h);T._af=requestAnimationFrame(f)})()};
...
P._cho=function(pr,choices){
 ...
 choices.forEach(function(c,idx){if(c.condition&&!T._cd(c.condition))return;
  ...
  b.onmouseenter=function(){b.style.borderColor='rgba(255,255,255,.3)';b.style.color='#fff'};
  b.onmouseleave=function(){b.style.borderColor='rgba(255,255,255,.1)';b.style.color='#c8c8d0'}})};
